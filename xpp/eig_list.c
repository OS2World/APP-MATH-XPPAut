...
create_eq_list()
{
 ...
 static char *iname[]={"Eqns"};
  ...
  eq_box.flag=0; /*  this is to tell that no eq_box is here  */ 
 ...
 eq_list.up=make_window(eq_list.main,10,5,2*DCURXs,DCURYs+2,1);
 eq_list.down=make_window(eq_list.main,10+4*DCURXs,5,4*DCURXs,DCURYs+2,1);
...
create_eq_box(cp,cm,rp,rm,im,y,n)
