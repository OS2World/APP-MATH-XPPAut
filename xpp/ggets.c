...
char *string;
{
 if(Xup) respond_box(main_win,0,0,"OK",string);
 else { printf("%s\n",string);}
}
     ...
     XKeyEvent *ev;
{
  ...
  if((ks==XK_Return)||(ks==XK_KP_Enter)||
     (ks==XK_Linefeed))return(13);
  else if	(((ks>=XK_KP_Space)&&(ks<=XK_KP_9))
		 || ((ks>=XK_space)&&(ks<=XK_asciitilde)))
    return((int)buf[0]);
  else if ((ks>=XK_Shift_L)&&(ks<=XK_Hyper_R)) return(0);
  else if ((ks>=XK_F1)&&(ks<=XK_F35))  return(0);
  /*      else if ((ks==XK_BackSpace)||(ks==XK_Delete))return(8);   */
  else if (ks==XK_BackSpace) return(8);
  else if (ks==XK_Delete) return(9);
  else if (ks==XK_Tab) return(10);
  else
    {
      ping();
      return(0);
