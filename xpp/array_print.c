...
array_print(filename,xtitle,ytitle,nacross,ndown,col0,row0,nskip,maxrow,maxcol,
	    data,zmin,zmax,tlo,thi,type)
     char *xtitle,*ytitle,*filename;
     int nacross,ndown;
     int col0,row0,nskip,maxrow,maxcol;
     int type;
     float **data;
     double zmin,zmax,tlo,thi;
{
  ...
  ps_replot(data,col0,row0,nskip,maxrow,maxcol,nacross,ndown,zmin,zmax,type);
  ps_boxit(tlo,thi,0.0,yy,zmin,zmax,xtitle,ytitle,type);
  ps_close();
  return 0;
 }


 ps_replot(z,col0,row0,nskip,maxrow,maxcol,nacross,ndown,zmin,zmax,type)
      float **z;
      double zmin,zmax;
      int ndown,nacross,maxcol,maxrow,row0,col0,nskip,type;
 {
   int i,j,ib,jb;
   ...
   float xlo=.15*dx,ylo=.15*dy,xhi=.95*dx,yhi=.95*dy;
  float delx,dely;
  delx=.8*dx/(float)ndown;
  dely=.8*dy/(float)nacross;
  for(i=0;i<nacross;i++){
    ib=col0+i;
    if(ib>maxcol)return;
    for(j=0;j<ndown;j++){
...
ps_col_scale(y0,x0,dy,dx,n,zlo,zhi,type)
     ...
     int n,type;
{
  ...
  sprintf(s,"%g",zlo);
  ps_text2(s,x0+.5*dx,y0+.5*ps_scale.lety*(1+strlen(s)),1);
    sprintf(s,"%g",zhi);
  ps_text2(s,x0+.5*dx,y0-n*dy-dy/2,0);
}

ps_boxit(tlo,thi,jlo,jhi,zlo,zhi,sx,sy,type)
     ...
     char *sx,*sy;
{
  ...
  float xlo=.15*dx,ylo=.15*dy,xhi=.95*dx,yhi=.95*dy;
  ps_setline(0.0,10);
  ps_rect(.15*dx,.15*dy,.8*dx,.8*dy);	  
  ps_setline(z,i);
  ps_text2(sx,xhi+.01*dx,.5*(yhi+ylo)+.4*strlen(sx)*ps_scale.lety,0);
  ps_text2(sy,.5*(xhi+xlo),yhi+.8*strlen(sy)*ps_scale.lety,0);
  sprintf(str,"%g",tlo);
  ps_text2(str,xhi-.01*dx,yhi+.8*strlen(str)*ps_scale.lety,0);
  sprintf(str,"%g",thi);
  ps_text2(str,xlo,yhi+.8*strlen(str)*ps_scale.lety,0);
  sprintf(str,"%g",jlo);
  ps_text2(str,xhi+.01*dx,yhi+.8*strlen(str)*ps_scale.lety,0);
  sprintf(str,"%g",jhi);
  ps_text2(str,xhi+.01*dx,ylo+.8*strlen(str)*ps_scale.lety,0);
