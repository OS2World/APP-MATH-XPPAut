...
extern int NFlags;
extern double STOL;
extern char *info_message,*meth_hint[],*color_hint[],*map_hint[];
   ...
   switch(ch){
		...
		case 'e': flash(9);
			...
			new_float("Maximal delay :",&DELAY);
                        new_float("AlphaMax :", &AlphaMax);
		        new_float("OmegaMax :", &OmegaMax);
...
get_method()
{
 ...
 static char *n[]={"(D)iscrete","(E)uler","(M)od. Euler",
	"(R)unge-Kutta","(A)dams","(G)ear","(V)olterra","(B)ackEul",
		 "(Q)ualst.RK4","(S)tiff","(C)Vode"};
 static char key[]="demragvbqsc";
 ...
 nmeth=10;
#endif 
 ch = (char)pop_up_list(&temp,"Method",n,key,nmeth,14,METHOD,10,10*DCURY+8,
  ...
  set_col_par()
   {
	...
	3,11,MyGraph->ColorFlag,10,12*DCURY+8,color_hint,
