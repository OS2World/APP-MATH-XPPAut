     ...
     BROWSER *b;
{
  ...
  if((my_ode[NEQ+FIX_VAR]=(int *)malloc(400))==NULL){
     ...
     BROWSER b;
{
  int i,npts,ok;
  FILE *fp;
  open_write_file(&fp,filename,&ok,b.main);
  if(!ok)return;
...
/* if(NODE==1){
   sprintf(temp,"%s#",uvar_names[0]);
   add_stor_col(temp,"0",&my_browser);
 } */
}
