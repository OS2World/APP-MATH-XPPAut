     ...
     int n,i0,iend,ishift;
{
   ...
   for(l=0;l<nvar;l++)Set_ivar(l+1,Memory[l][ishift]);
   ...
   for(i=1;i<=iend;i++){
     ...
     tp+=dt;
     Set_ivar(PrimeStart,tp);
     for(l=0;l<nvar;l++)Set_ivar(l+1,Memory[l][ioff]);
     for(ker=0;ker<NKernel;ker++){
       mu=kernel[ker].mu;
       if(mu==0.0)alpbet=dt;
     ...
     int nt,neq,*istart;
{
  ...
  if(*istart==1){
    ...
    Set_ivar(PrimeStart,*t);
    for(i=0;i<NODE;i++)
      if(!EqType[i])Set_ivar(i+1,y[i]);  /* assign initial data             */
    for(i=NODE;i<NODE+FIX_VAR;i++)
