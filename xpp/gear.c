...
int rooter(/* double *x,double err,double eps,double big,
double *work,int *ierr,int maxit,int n */);


int do_sing(/* double *x,double eps,double err,double big,
	       int maxit,int n,int *ierr,float *stabinfo */);
int pr_evec(/* double *x,double *ev,int n,int pr,double eval */);
int get_evec(/* double *a,double *anew,double *b,double *bp,int n,int maxit,
		double err,int *ipivot,double eval, int *ierr */);
int eq_symb(/* double *,int */);
double amax(/* double,double */);
double sign(/* double,double */);
int eigen(/* int,double *,double *,double *,int * */);
int orthes(/* int,int,int,double *,double * */);
int hqr(/* int,int,int,double *,double *,int * */);
int getjac(/* double *x,double *y,double *yp,double *xp,
	      double eps,double *dermat,int n */);
char status();
int my_rhs(/* double ,double *,double *,int */);
int sscal(/* int n,double sa,double *sx,int incx */);
...
int sgesl(/* double *a,int lda,int n,int *ipvt,double *b,int job */);
int sgefa(/* double *a,int lda,int n,int *ipvt,int *info */);
int gear(/* int n,double *t,double tout,double *y,double hmin,
...
if(!PAR_FOL)
{
 ch=(char)two_choice("YES","NO","Print eigenvalues?","yn",0,0,main_win);
 ...
 *stabinfo=(float)(cp+rp)+(float)(cn+rn)/100.0;
 ...
 if(!PAR_FOL)
 {
  ch=(char)two_choice("YES","NO","Draw Invariant Sets?","yn",0,0,main_win);
   }
  if((ch=='y')||(PAR_FOL&&SHOOT))
  {
   ...
   if(rp==1)
   {
     get_evec(oldwork,work,b,bp,n,maxit,err,ipivot,eval[2*pose],ierr);
     if(*ierr==0)
     {
...
double eval;
{
 ...
 int n,maxit,  *ipivot, *ierr;
    ...
    *ierr=0;
    ...
    anew[j]=a[j];
    for(j=0;j<n;j++)
    anew[j*(1+n)]=anew[j*(1+n)]-eval-err*err*fabs(eval);
...
ggear( n,t, tout,y, hmin, hmax,eps,
     mf,error,kflag,jstart,work,iwork)
 int n,mf,*kflag,*jstart,*iwork;
double *t, tout, *y, hmin, hmax, eps,*work,*error;
