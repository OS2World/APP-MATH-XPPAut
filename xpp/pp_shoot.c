...
extern int *my_ode[];
extern char *info_message, *bvp_hint[];
 ...
 int n;
{
 ...
 for(i=0;i<n;i++){
  ...
  for(i=n;i<n+FIX_VAR;i++)Set_ivar(i+1,evaluate(my_ode[i]));
...
double *sect;
{
 static char *n[]={"Freq. Par.","Sect. Var","Section","Show(Y/N)"};
...
find_bvp()
{
 ...
 ch=(char)pop_up_list(&temp,"Bndry Value Prob",n,key,4,12,1,10,6*DCURY+8,
