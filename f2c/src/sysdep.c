...
#else
char *c_functions	= "c_funct";
char *coutput		= "c_output";
char *initfname		= "raw_data";
char *initbname		= "raw_data.b";
char *blkdfname		= "block.dat";
char *p1_file		= "p1_file";
char *p1_bakfile	= "p1_file.BAK";
char *sortfname		= "initfile";
char *proto_fname	= "protfile";
#endif
...
#else
	int pid = getpid();
	sprintf(c_functions, "%s/f2c%d.fnc", tmpdir, pid);
	sprintf(initfname, "%s/f2c%d.rd", tmpdir, pid);
	sprintf(blkdfname, "%s/f2c%d.blk", tmpdir, pid);
	sprintf(p1_file, "%s/f2c%d.p1f", tmpdir, pid);
	sprintf(p1_bakfile, "%s/f2c%d.p1b", tmpdir, pid);
	sprintf(sortfname, "%s/f2c%d.srt", tmpdir, pid);
#endif
